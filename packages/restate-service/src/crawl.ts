...
async function submitCrawlJob(url: string): Promise<string> {
  ...
  const res = await fetch(endpoint, {
    ...
    body: JSON.stringify({ url, depth: 1, limit: 1, render: false, formats: ["markdown"] })
  });
...
export async function crawlSite(
  ...
  const jobId = await ctx.run("submitCrawl", () => submitCrawlJob(url));

  const maxAttempts = 40;
  for (let i = 0; i < maxAttempts; i++) {
    await ctx.sleep(5000);
    const poll = await ctx.run(`poll-${i}`, () => pollCrawlJob(jobId));
